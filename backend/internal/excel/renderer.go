...
func (r *Renderer) Render(invoice domain.Invoice) ([]byte, error) {
	f := excelize.NewFile()
	...
	titleStyle,
		...
		subtotalHoursStyle,
		totalStyle,
		totalHoursStyle := r.styles(f)

	row := 1
	var subtotalCells []string

	for _, group := range invoice.Groups {

		blockTitle := fmt.Sprintf(
			"%s - %s",
			invoice.Company.NameShort,
			group.Label,
		)

		// === Block title ===
		titleCell := fmt.Sprintf("A%d", row)
		f.SetCellValue(sheet, titleCell, blockTitle)
		...
		for _, e := range group.Rows {
			...
			f.SetCellValue(sheet, a, e.Date.Format("02.01.2006"))
			f.SetCellValue(sheet, b, e.TicketCode)
			f.SetCellValue(sheet, c, e.Hours)
		...
		f.SetCellValue(sheet, subtitleCell, "Subtotal "+blockTitle)
...
func (r *Renderer) styles(f *excelize.File) (
	title,
	header,
	cellDate,
	cellNotes,
